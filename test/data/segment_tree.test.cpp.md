...
data:
  ...
  bundledCode: "#define PROBLEM \"https://judge.yosupo.jp/problem/point_add_range_sum\"\
    ...
    \ n_leaf(bit_ceil(n)), data(n_leaf * 2, A::id) {}\n    SegmentTree(const vector<T>&\
    \ v) : _n(v.size()), n_leaf(bit_ceil(_n)), data(n_leaf * 2, A::id) {\n       \
    \ std::copy(v.begin(), v.end(), data.begin() + n_leaf);\n        for (int i =\
    \ n_leaf - 1; i > 0; --i) data[i] = A::op(data[i * 2], data[i * 2 + 1]);\n   \
    \ }\n\n    T query(int l, int r) const {\n        assert(0 <= l && l < _n);\n\
    \        assert(l <= r);\n        assert(r <= _n);\n\n        T left = A::id,\
    \ right = A::id;\n        for (l += n_leaf, r += n_leaf; l < r; l >>= 1, r >>=\
    \ 1) {\n            if (l & 1) left = A::op(left, data[l++]);\n            if\
    \ (r & 1) right = A::op(data[--r], right);\n        }\n        return A::op(left,\
    \ right);\n    }\n\n    T query_all() const { return data[1]; }\n\n    void update(int\
    \ i, T val) {\n        assert(0 <= i && i < _n);\n        i += n_leaf;\n     \
    \   data[i] = val;\n        for (i >>= 1; i > 0; i >>= 1) data[i] = A::op(data[i\
