...
data:
  ...
  bundledCode: "#include <stack>\n\n#include <cstdint>\nnamespace bys {\nusing i8\
    ...
    \ = EdgesCSR<Edge<>>;\n}  // namespace bys\n\nnamespace bys {\ntemplate <class\
    \ E> auto dfs(EdgesCSR<E> const& graph, typename E::vertex_type source) {\n  \
    \  using V = typename E::vertex_type;\n    std::vector<bool> seen(graph.size());\n\
    \    std::vector<V> res;\n    std::stack<V> st;\n    st.push(source);\n    while\
    \ (not st.empty()) {\n        auto top = st.top();\n        st.pop();\n      \
    \  seen[top] = true;\n        res.push_back(top);\n        for (auto&& e : graph[top])\
    \ {\n            if (not seen[e.dest]) {\n                st.push(e.dest);\n \
    \           }\n        }\n    }\n}\ntemplate <class E> auto pre_order(const EdgesCSR<E>&\
    \ graph) {\n    using V = typename E::vertex_type;\n    i32 n = graph.size();\n\
    \    std::vector<bool> seen(n);\n    std::vector<V> res;\n    FixPoint pre_dfs([&](auto&&\
    \ self, V now) -> void {\n        seen[now] = true;\n        res.push_back(now);\n\
    \        for (auto&& e : graph[now]) {\n            if (not seen[e.dest]) self(e.dest);\n\
    \        }\n    });\n    for (i32 i = 0; i < n; ++i) {\n        if (not seen[i])\
    \ pre_dfs(i);\n    }\n    return res;\n}\n\n// std::vector<i32> in_order(const\
    \ EdgesCSR& graph, i32 source);\ntemplate <class E> auto post_order(const EdgesCSR<E>&\
    \ graph) {\n    using V = typename E::vertex_type;\n    i32 n = graph.size();\n\
    \    std::vector<bool> seen(n);\n    std::vector<V> res;\n    FixPoint post_dfs([&](auto&&\
    \ self, V now) -> void {\n        seen[now] = true;\n        for (auto&& e : graph[now])\
    \ {\n            if (not seen[e.dest]) self(e.dest);\n        }\n        res.push_back(now);\n\
    \    });\n    for (i32 i = 0; i < n; ++i) {\n        if (not seen[i]) post_dfs(i);\n\
    \    }\n    return res;\n}\n// std::vector<i32> euler_tour(const EdgesCSR& graph,\
    \ i32 source);\n}  // namespace bys\n"
  code: "#pragma once\n#include <stack>\n\n#include \"../core/int_alias.hpp\"\n#include\
    \ \"../extension/fixpoint.hpp\"\n#include \"graph.hpp\"\n\nnamespace bys {\ntemplate\
    \ <class E> auto dfs(EdgesCSR<E> const& graph, typename E::vertex_type source)\
    \ {\n    using V = typename E::vertex_type;\n    std::vector<bool> seen(graph.size());\n\
    \    std::vector<V> res;\n    std::stack<V> st;\n    st.push(source);\n    while\
    \ (not st.empty()) {\n        auto top = st.top();\n        st.pop();\n      \
    \  seen[top] = true;\n        res.push_back(top);\n        for (auto&& e : graph[top])\
  ...
  verifiedWith: []
documentation_of: byslib/graph/dfs.hpp
layout: document
redirect_from:
- /library/byslib/graph/dfs.hpp
- /library/byslib/graph/dfs.hpp.html
