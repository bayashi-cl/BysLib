...
template <class... Args> class operative_tuple : public std::tuple<Args...> {
  ...
  public:
    template <class T> operative_tuple(T const& v) noexcept {
...
#define DEFINE_BINOP(op, iop, name)                                                          \
  ...
  public:                                                                                    \
    ...
    friend operative_tuple operator op(operative_tuple const& a, operative_tuple const& b) { \
        return operative_tuple(a) iop b;                                                     \
    }
...
#define DEFINE_MONOOP(op, name)                                                 \
  private:                                                                      \
    template <std::size_t... I> auto name(std::index_sequence<I...>) noexcept { \
...
#define DEFINE_INCL(op, name)                                                   \
  private:                                                                      \
    template <std::size_t... I> void name(std::index_sequence<I...>) noexcept { \
        (op std::get<I>(*this), ...);                                           \
    }                                                                           \
                                                                                \
  public:                                                                       \
...
#define DEFINE_BITSHIFT(op, iop, name)                                                         \
  ...
  public:                                                                                      \
    ...
    friend operative_tuple operator op(operative_tuple const& tp, std::size_t x) {             \
        return operative_tuple(tp) iop x;                                                      \
    }
