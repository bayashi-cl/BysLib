...
data:
  ...
  code: "#pragma once\n#include <cstddef>\n#include <limits>\n#include <tuple>\n#include\
    ...
    \ntemplate <class T> constexpr T get_inf();\nnamespace impl {\ntemplate <class\
    \ Tp, std::size_t... I> constexpr auto get_inf_tuple(std::index_sequence<I...>)\
    \ {\n    return Tp{get_inf<typename std::tuple_element_t<I, Tp>>()...};\n}\n}\
    \  // namespace impl\ntemplate <class T> constexpr T get_inf() {\n    if constexpr\
    \ (std::is_integral_v<T>) {\n        return std::numeric_limits<T>::max() / (T)2;\n\
    \    } else if constexpr (std::is_floating_point_v<T>) {\n        return std::numeric_limits<T>::infinity();\n\
...
documentation_of: byslib/core/constant.hpp
layout: document
redirect_from:
- /library/byslib/core/constant.hpp
- /library/byslib/core/constant.hpp.html
title: Const
---
